...
// Este script ya no se va a usar, queda ahí para borrar luego si por si las dudas
...
public class SubeYBajaScript : MonoBehaviour
{
    ...
    // Coroutine para la translación desde la punta
    ...
    // Coroutine para la translación desde la base
    ...
    private IEnumerator TranslacionBase()
    {
        ...
        if (seleccionado != null && seleccionado.name == "Cilindro" && !puntaBool)
        {
            ...
            if (rollTo != null)
            {
                ...
                // Espera breve para asegurar alineaci�n del socket (ajusta si es necesario)
